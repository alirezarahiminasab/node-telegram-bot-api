...
var TelegramBot = (function (_EventEmitter) {
    ...
    [
      {
        ...
        value: function on(event, listener) {
          if (_deprecatedMessageTypes.indexOf(event) !== -1) {
            ...
            deprecate(
              "Events " +
                _deprecatedMessageTypes.join(",") +
                " are deprecated. See the updated list of events: " +
                url
            );
          }
          _get(
            TelegramBot.prototype.__proto__ ||
              Object.getPrototypeOf(TelegramBot.prototype),
            "on",
            this
  ...
  function TelegramBot(token) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (TelegramBot.__proto__ || Object.getPrototypeOf(TelegramBot)).call(this)
    );

    _this.token = token;
    _this.options = options;
    _this.options.polling =
      typeof options.polling === "undefined" ? false : options.polling;
    _this.options.webHook =
      typeof options.webHook === "undefined" ? false : options.webHook;
    _this.options.baseApiUrl = options.baseApiUrl || "https://tapi.bale.ai";
    _this.options.filepath =
      typeof options.filepath === "undefined" ? true : options.filepath;
    _this.options.badRejection =
  ...
  _createClass(TelegramBot, [
      ...
      value: function _formatSendData(type, data) {
        ...
        if (data instanceof stream.Stream) {
          ...
          if (!filename && !process.env.NTBA_FIX_350) {
            deprecate(
              'Buffers will have their filenames default to "filename" instead of "data". ' +
                deprecationMessage
            );
            filename = "data";
          }
          if (!contentType) {
            ...
            if (filetype) {
              ...
              deprecate(
                "An error will no longer be thrown if file-type of buffer could not be detected. " +
                  deprecationMessage
              );
      ...
      value: function getFileStream(fileId) {
        ...
        this.getFileLink(fileId, form)
          .then(function (fileURI) {
            ...
            pump(
              streamedRequest(
                Object.assign({ uri: fileURI }, _this3.options.request)
              ),
              fileStream
            );
          })
    ...
    },
    {
      ...
      value: function removeTextListener(regexp) {
        var index = this._textRegexpCallbacks.findIndex(function (
          textListener
        ) {
    ...
    },
    {
      ...
      value: function processUpdate(update) {
        ...
        if (message) {
          ...
          debug(
            "Process Update edited_business_message %j",
          ...
          debug(
            "Process Update message_reaction_count %j",
      ...
      value: function setWebHook(url) {
        ...
        if (
          (typeof options === "undefined" ? "undefined" : _typeof(options)) !==
            "object" ||
          options instanceof stream.Stream
        ) {
          deprecate(
        ...
        if (cert) {
          try {
            var sendData = this._formatSendData(
              "certificate",
              cert,
              fileOptions
            );
    ...
    },
    {
      ...
      value: function sendMediaGroup(chatId, media) {
        ...
        try {
          for (
            var _iterator = media[Symbol.iterator](), _step;
            ...
            try {
              ...
              var _formatSendData4 = this._formatSendData(
                  attachName,
                  input.media,
                  input.fileOptions
                ),
    ...
    },
    {
      ...
      value: function answerCallbackQuery(callbackQueryId) {
         ...
         * consistency of the method signatures throughout the library. */
        if (
          (typeof callbackQueryId === "undefined"
            ? "undefined"
            : _typeof(callbackQueryId)) === "object"
        ) {
          /* eslint-disable no-param-reassign, prefer-rest-params */
          deprecate(
      ...
      value: function editMessageMedia(media) {
        ...
        if (typeof media.media === "string" && regexAttach.test(media.media)) {
          ...
          try {
            ...
            var _formatSendData6 = this._formatSendData(
                attachName,
            ...
            if (formData) {
              ...
              throw new errors.FatalError(
                "Failed to process the replacement action for your " +
                  media.type
              );
            }
          } catch (ex) {
    ...
    },
    {
      ...
      value: function addStickerToSet(userId, name, sticker, emojis) {
        ...
        if (
          ...
          ["png_sticker", "tgs_sticker", "webm_sticker"].indexOf(
            stickerType
          ) === -1
        ) {
          return Promise.reject(
    ...
    },
    {
      ...
      value: function createInvoiceLink(
        title,
        description,
        payload,
        providerToken,
